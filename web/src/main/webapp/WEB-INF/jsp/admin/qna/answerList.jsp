        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">답변완료 등록수 <span class="badge bg-green">${map.countList }</span> </h3>
   ...
   <c:forEach items="${map.list }" var="row">
		...
		<td>
		  <c:choose>
		  	...
		  	<c:otherwise>
		  	 &nbsp;&nbsp;&nbsp;&nbsp;<span class="badge bg-red">${row.idx }</span>
		  	</c:otherwise>
		  </c:choose>	
		</td>
		...
		<td>
		  <c:choose>
		  	<c:when test="${row.userid=='braverokmc' }">
		  	관리자
		  	</c:when>
		  	<c:otherwise>
		  	${row.userid }
		...
		<td>
		  <c:choose>
		  	<c:when test="${row.depth==0 }">
		  	<a href="/admin/qna/read/${row.idx}">${row.subject }</a>
		  	</c:when>
		  	<c:otherwise>
...
function list(page){
	...
	location.href='/admin/qna/answerList'+page;
